...
const viewFlaggedAnswers = async (req, res) => {
  try {
    let role = preprocessing(req.userRole);
    if (role == "moderator") {
      let answers = await Answer.find({ flag: true }).sort({
        ans_date_time: -1,
      });
...
const viewFlaggedComments = async (req, res) => {
  try {
    let role = preprocessing(req.userRole);
    if (role == "moderator") {
      let comments = await Comment.find({ flag: true }).sort({
        comment_date_time: -1,
      });
...
const deleteFlaggedQuestion = async (req, res) => {
  try {
    ...
    if (qid == "") {
      res
        .status(422)
        .json({ error: "Bad input received. Cannot receive empty qid." });
    }
    if (role == "moderator") {
    ...
    res
      .status(500)
      .json({ error: `Unable to delete flagged question: ${err}` });
  }
};
...
const deleteFlaggedAnswer = async (req, res) => {
  try {
    ...
    if (aid == "") {
      res
        .status(422)
        .json({ error: "Bad input received. Cannot receive empty aid." });
    }
    if (role == "moderator") {
      ...
      for (let comment in answer["comments"]) {
        await Comment.deleteOne({
          _id: answer["comments"][comment].toString(),
        });
      }
...
const deleteFlaggedComment = async (req, res) => {
  try {
    ...
    if (cid == "") {
      res
        .status(422)
        .json({ error: "Bad input received. Cannot receive both empty cid." });
    }
    if (role == "moderator") {
